...
class EntityMapper:
    def __init__(self, mapping_config_file):
        ...
        with open(mapping_config_file, mode='rt', encoding='utf-8') as config_file:
            ...
            for config_item in config_params:
                self.validateConfigItem(config_item)
                self.feature_mapping[config_item['SENZING_ENTITY_TYPE']] = config_item
    ...
    def validateConfigItem(self, config_item):
        ...
        if 'SENZING_ENTITY_TYPE' not in config_item:
            raise Exception(F'Invalid mapping config item.  Missing required field SENZING_ENTITY_TYPE {config_item}')
